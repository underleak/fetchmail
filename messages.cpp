...
void Messages::display (QListWidgetItem * msg)
{
    ...
    while (database->query.next()) {
        ...
        if(counter==(listWidget->row(msg)+1))
            ui->textBrowser->setText(" Theme: " +
            database->query.value(database->queryRecord.indexOf("theme")).toString()+ "\n From:" +
            database->query.value(database->queryRecord.indexOf("from")).toString() + "\n To:" +
            database->query.value(database->queryRecord.indexOf("to")).toString() + "\n Date:  " +
            database->query.value(database->queryRecord.indexOf("date")).toString() + "\n\n" +
            database->query.value(database->queryRecord.indexOf("text")).toString());
    }
    //qDebug()<<this->acc_id;
...
void Messages::refresh()
{
    ...
    while (database->query.next()) {
        ...
        newItem->setText( "\n Theme: " + database->query.value(database->queryRecord.indexOf("theme")).toString()+ "\n From:" +
                          database->query.value(database->queryRecord.indexOf("from")).toString() +
                          "\n Date:  " + database->query.value(database->queryRecord.indexOf("date")).toString() + "\n");
    ...
    ui->loginLabel->setText("Почта: " +  database->query.value(database->queryRecord.indexOf("to")).toString());
...
{
     QString path = QFileDialog::getOpenFileName(this, "Open Dialog", "", "*.png *.jpg *.bmp *.JPG");
